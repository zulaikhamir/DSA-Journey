...
*/
//if i want to sort an array using lomuto partition, i can use the quicksort algorithm which is a divide and conquer algorithm that uses the partitioning logic to sort the array. The quicksort algorithm works by selecting a pivot element, partitioning the array around the pivot, and then recursively sorting the subarrays on either side of the pivot. Here is an example of how to implement quicksort using the Lomuto partition scheme:but using only basic idea of lomuto which is in this code can not be used to sort the array entirely,we need quicksort for that
