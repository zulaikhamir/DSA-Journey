...
class Solution {
    public int minimumDifference(int[] nums, int k) {
        ...
        for (int i = 0; i <= nums.length - k; i++) {//why -k? because we are checking the difference between the k-1 th element and the 0th element, then the k th element and the 1st element and so on until we reach the end of the array. so we need to stop at nums.length - k otherwise we will get an ArrayIndexOutOfBoundsException.
